...
public class CoreSchedulerServiceTest extends AppFabricTestBase {
  ...
  public void testRunScheduledJobs() throws Exception {
    ...
    MessageId messageId = waitUntilProcessed(dataEventTopic, null);
    publishNotification(dataEventTopic, WORKFLOW_2, AppWithFrequentScheduledWorkflows.DATASET_NAME2);
    messageId = waitUntilProcessed(dataEventTopic, messageId);
    publishNotification(dataEventTopic, WORKFLOW_2, AppWithFrequentScheduledWorkflows.DATASET_NAME2);
    ...
    enableSchedule(AppWithFrequentScheduledWorkflows.DATASET_PARTITION_SCHEDULE_2);
    testScheduleUpdate("disable", getLastMessageId(dataEventTopic));
    testScheduleUpdate("update", getLastMessageId(dataEventTopic));
    testScheduleUpdate("delete", getLastMessageId(dataEventTopic));
  }

  private void testScheduleUpdate(String howToUpdate, MessageId messageId) throws Exception {
    ...
    publishNotification(dataEventTopic, WORKFLOW_2, AppWithFrequentScheduledWorkflows.DATASET_NAME2);
    waitUntilProcessed(dataEventTopic, messageId);
    Assert.assertEquals(runs, getRuns(WORKFLOW_2));
  ...
  @Nullable
  private MessageId waitUntilProcessed(final TopicId topic, @Nullable final MessageId messageId) throws Exception {
    // Wait for the persisted message changed. That means the scheduler actually consumed the last data event
    final AtomicReference<MessageId> newMessageId = new AtomicReference<>();
    Tasks.waitFor(true, new Callable<Boolean>() {
      @Override
      public Boolean call() throws Exception {
        newMessageId.set(getLastMessageId(topic));
        return !Objects.equals(messageId, newMessageId.get());
      }
    }, 5, TimeUnit.SECONDS);
    return newMessageId.get();
  }
